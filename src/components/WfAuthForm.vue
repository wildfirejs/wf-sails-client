    ...
    handleSignIn (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          this.db.put('/signIn', {email, password}, ({ error, data }) => {
            ...
            // hook: signedIn
            const cbs = Bus.hooks.signedIn || []
            cbs.forEach(cb => cb({ bus: this.bus, user: data }))
          })
        } else {
    ...
    handleSignUp (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          this.db.post('/signUp', { email, password }, ({ data, error }) => {
            if (error) {
