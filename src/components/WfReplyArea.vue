  ...
  methods: {
    ...
    postComment () {
      ...
      if (content.trim() !== '') {
        ...
        const byUser = user ? user.uid : null

        const { pageURL, pageTitle } = this.config
        let parentComment = null
        let rootComment = null

        if (isReply) {
          parentComment = replyToComment.id
          if (replyToComment.rootComment) {
            rootComment = replyToComment.rootComment
          } else {
            rootComment = replyToComment.id
          }
        }
        ...
        this.db.post('/comment', postData, ({ error, data }) => {
          if (error) {
            this.isPosting = false
